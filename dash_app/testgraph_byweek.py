...
def dropdown_spendcat():
    ...
    return dcc.Dropdown(id='choosecat',
                        ...
                        value=tracker_df.at[1, 'category'])
...
app.layout = html.Div(children=[
    html.H1(children=[build_banner(),
                ...
                children='testing graph only output'),
            ]
    ),
       # add in dropdown for graph
       # then draw graph
    html.Div([html.H3("Select spending category to track: ",style={'fontSixe':20}),
            html.Div(children=[dropdown_spendcat()],style={'width':'40%', 'display':'inline-block'}),
            ]),
    html.Div([html.H3("Show spend amount for each: ",style={'fontSixe':20}),
            html.Div(children=[dropdown_usercat()],style={'width':'40%', 'display':'inline-block'}),
            ]),
    html.Div([html.H3("Select start and end date: ",style={'fontSixe':20}),
...
def update_graph(selected_cat,startdt_graph,enddt_graph):
    ...
    weekblocks=[]
    rgb_red=0
    for spendweek in tracker_df_cat['weekid'].unique():
        tracker_df_cat_week = tracker_df_cat[tracker_df_cat['weekid']==spendweek]
        grouped_df = tracker_df_cat_week.groupby("title", as_index=False).agg({"magnitude":"sum"}).sort_values("magnitude",ascending=True)
        rgb_red += 50
